...
const additional_model_1 = require("../models/additional.model");
class AdditionalService {
    // ── Milestones ─────────────────────────────────────────────────────────────
    /**
     * List milestones with optional filters
     */
    static async getMilestones(userId, filters) {
        ...
        const [milestones, total] = await Promise.all([
            prisma_1.default.milestone.findMany({
                where,
            ...
            prisma_1.default.milestone.count({ where }),
        ]);
        return {
            milestones: milestones.map(additional_model_1.toMilestoneResponse),
            pagination: {
                page,
                limit,
                total,
     ...
     */
    static async createMilestone(userId, data) {
        // Verify user has access to this repository
        const access = await prisma_1.default.userRepository.findFirst({
        ...
        return (0, additional_model_1.toMilestoneResponse)(milestone);
    }
    // ── Settings ───────────────────────────────────────────────────────────────
    /**
     ...
     */
    static async getActivityLog(userId, page = 1, limit = 20) {
        const skip = (page - 1) * limit;
        ...
        const [recentIssues, recentComments, totalIssues, totalComments] = await Promise.all([
            ...
            prisma_1.default.issue.count({ where: { creatorId: userId } }),
            prisma_1.default.comment.count({ where: { userId } }),
        ]);
    ...
    static async exportData(userId, options) {
        ...
        let filename = '';
        if (entity === 'issues') {
            const where = { repositoryId: { in: accessibleRepoIds } };
     ...
     */
    static async bulkAction(userId, request) {
        ...
        const accessibleRepoIds = userRepos.map((r) => r.repositoryId);
        const accessibleIssues = await prisma_1.default.issue.findMany({
            where: {
                id: { in: issueIds },
                repositoryId: { in: accessibleRepoIds },
            },
        ...
        switch (action) {
            case 'close':
                const closeResult = await prisma_1.default.issue.updateMany({
                ...
                break;
            case 'reopen':
                const reopenResult = await prisma_1.default.issue.updateMany({
                    where: { id: { in: accessibleIds } },
            ...
            case 'assign_label':
                if (!payload?.labelId)
                    throw new Error('labelId is required for assign_label action');
                // Upsert to avoid duplicate key errors
                for (const issueId of accessibleIds) {
                    await prisma_1.default.issueLabel.upsert({
            ...
            case 'remove_label':
                if (!payload?.labelId)
                    throw new Error('labelId is required for remove_label action');
                const removeLabelResult = await prisma_1.default.issueLabel.deleteMany({
            ...
            case 'assign_milestone':
                ...
                const milestoneResult = await prisma_1.default.issue.updateMany({
                    where: { id: { in: accessibleIds } },
            ...
            case 'set_priority':
                ...
                const priorityResult = await prisma_1.default.issue.updateMany({
                    where: { id: { in: accessibleIds } },
     ...
     */
    static async getTeams(page = 1, limit = 20) {
        const skip = (page - 1) * limit;
        ...
        return {
            teams: teams.map(additional_model_1.toTeamResponse),
            pagination: {
                page,
                limit,
                total,
    ...
    static async createTeam(userId, data) {
        ...
        return (0, additional_model_1.toTeamResponse)(team);
    }
    // ── Health ─────────────────────────────────────────────────────────────────
    /**
